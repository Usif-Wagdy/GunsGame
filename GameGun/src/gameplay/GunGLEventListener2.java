...
public class GunGLEventListener2 extends GunListener {
    ...
    Directions2 direction2 = Directions2.right;
    int soldierIndex = 0;
    int soldierIndex2 = 0;
    int maxWidth = 100;
    int maxHeight = 100;
    ...
    int health = 100; // Initial health of the soldier
    public int timer = 0;
    ...
    String textureNames[] = {"Man1.png", "Man2.png", "Man3.png", "Man4.png",
            "Fire.png",
            "Walk (1).png", "Walk (2).png", "Walk (3).png", "Walk (4).png", "Walk (5).png", "Walk (6).png", "Walk (7).png", "Walk (8).png", "Walk (9).png", "Walk (10).png",
            "go_1.png", "go_2.png", "go_3.png", "go_4.png", "go_5.png", "go_6.png", "go_7.png", "go_8.png", "go_9.png", "go_10.png",
            "female1.png", "female2.png", "female3.png", "female4.png", "female5.png", "female6.png", "female7.png", "female8.png", "female9.png", "female10.png"
            , "0.png", "1.png", "2.png", "3.png", "4.png", "5.png", "6.png", "7.png", "8.png", "9.png","win.png","over.png", "blood.png", "background 3.png"};
...
//                mipmapsFromPNG(gl, new GLU(), texture[i]);
        ...
        if (GameisRunning) {
            ...
            for (Bullet2 bullet2 : bullets2) {
                if (bullet2.isFired) {
                    ...
                    DrawSprite2(gl, bullet2.x, bullet2.y, 4, 0.3f, bullet2.directions);
                }
            }
            ...
            for (Zombie zombie : zombies1) {
                ...
                if (checkCollisionWithSoldier2(zombie)) {
                    playSound("src\\Assets\\sounds\\zombieBite.wav");
                }
    ...
    public void DrawSprite2(GL gl, double x, double y, int index, float scale, Directions2 dir) {
        gl.glEnable(GL.GL_BLEND);
        gl.glBindTexture(GL.GL_TEXTURE_2D, textures[index]);    // Turn Blending On
    ...
    public void DrawSprite3(GL gl, double x, double y, int index, float scale, Directions2 dir) {
        gl.glEnable(GL.GL_BLEND);
        gl.glBindTexture(GL.GL_TEXTURE_2D, textures[index]);    // Turn Blending On
     ...
     * KeyListener
     */
    boolean canShoot1 = true;
    boolean canShoot2 = true;

    public void handleKeyPress() {
        // controls of bullets
        if (isKeyPressed(KeyEvent.VK_SPACE) && canShoot1) {
        ...
        if (!isKeyPressed(KeyEvent.VK_SPACE)) canShoot1 = true;
    ...
    public void handleKeyPress2() {
        // controls of bullets2
        if (isKeyPressed(KeyEvent.VK_F) && canShoot2) {
            double bulletX2 = soldierX2;
            double bulletY2 = soldierY2;
            ...
            bullets2.add(new Bullet2(direction2, bulletX2, bulletY2));
            playSound("src\\Assets\\sounds\\pistol.wav");
            canShoot2 = false;
        }
        if (!isKeyPressed(KeyEvent.VK_F)) canShoot2 = true;


        // controls of slodier2
        if (isKeyPressed(KeyEvent.VK_S) && isKeyPressed(KeyEvent.VK_A)) {
            if (soldierX2 > 0) {
                soldierX2--;
            }
            if (soldierY2 > 0) {
            ...
            direction2 = Directions2.down_left;
            soldierIndex2++;
        } else if (isKeyPressed(KeyEvent.VK_S) && isKeyPressed(KeyEvent.VK_D)) {
            if (soldierX2 < maxWidth - 10) {
                soldierX2++;
            }
            if (soldierY2 > 0) {
                soldierY2--;
            }
            direction2 = Directions2.down_right;
            soldierIndex2++;

        } else if (isKeyPressed(KeyEvent.VK_W) && isKeyPressed(KeyEvent.VK_A)) {
            if (soldierY2 < maxHeight - 37) {
                soldierY2++;
            }
            if (soldierX2 > 0) {
                soldierX2--;
            }
            direction2 = Directions2.up_left;
            soldierIndex2++;
        } else if (isKeyPressed(KeyEvent.VK_W) && isKeyPressed(KeyEvent.VK_D)) {
            if (soldierY2 < maxHeight - 37) {
                soldierY2++;
            }
            if (soldierX2 < maxWidth - 10) {
            ...
            direction2 = Directions2.up_right;
            soldierIndex2++;
    ...
    private void handleCollisions2() {
        // List to store zombies and bullets to be removed
        ArrayList<Zombie> zombiesToRemove = new ArrayList<>();
        ArrayList<Bullet2> bulletsToRemove2 = new ArrayList<>();
        ...
        for (Bullet2 bullet2 : bullets2) {
            if (bullet2.isFired) {
                // Check collision with zombies1
                for (Zombie zombie : zombies1) {
                    if (checkCollision2(bullet2, zombie)) {
                        zombiesToRemove.add(zombie);
                        bulletsToRemove2.add(bullet2);
                        bloodEffects.add(new BloodEffect(zombie.x, zombie.y, 60)); // Add blood effect
                        counter++; // Increase score
                        score++;
                        playSound("src\\Assets\\sounds\\zombiehit.wav");
        ...
        zombies2.removeAll(zombiesToRemove);
        zombies3.removeAll(zombiesToRemove);
        bullets.removeAll(bulletsToRemove2);
        regenerateZombies();
    }
    ...
    private boolean checkCollision2(Bullet2 bullet, Zombie zombie) {
        return Math.abs(bullet.x - zombie.x) < 5 && Math.abs(bullet.y - zombie.y) < 5;
    }
    ...
    private boolean checkCollisionWithSoldier2(Zombie zombie) {
        return Math.abs(zombie.x - soldierX2) < 5 && Math.abs(zombie.y - soldierY2) < 5;
    }
